...
class SlurmExecutorTest extends Specification {
    ...
    def testParseJob() {
        ...
        given:
        def exec = new SlurmNoMemoryExecutor()
...
def 'test job script headers' () {
        ...
        setup:
        // SLURM executor
        def executor = new SlurmNoMemoryExecutor()
        ...
        // If the memory exceeds the default memory per CPU (4 GB), and if the CPU is greater than 1 
        // and this is specified in the header
        when:
        task.config = new TaskConfig()
        task.config.time = '1h'
        task.config.memory = '16000 M'
        task.config.clusterOptions = '-a 1 --signal=KILL'
        then:
        executor.getHeaders(task) == '''
                #SBATCH -J nf-the_task_name
